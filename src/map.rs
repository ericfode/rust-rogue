...
use rltk::{
    Algorithm2D, BaseMap, FontCharType, RandomNumberGenerator, Rect, Rltk, RGB, Point,
};
use specs::Entity;
...
impl TileMeta for TileType {
    fn is_blocker(&self) -> bool {
        match self {
            TileType::Wall => true,
            TileType::CorWall => true,
            _ => false,
        }
    }
    fn glpyh(&self) -> FontCharType {
        match self {
            ...
            TileType::CorWall => rltk::to_cp437('#'),
            TileType::Floor => rltk::to_cp437('.'),
            TileType::Empty => rltk::to_cp437(' '),
        }
    }
    fn color(&self) -> RGB {
        match self {
            TileType::Wall => RGB::named(rltk::YELLOW),
            TileType::CorWall => RGB::named(rltk::GREEN),
            TileType::Floor => RGB::named(rltk::LIGHTBLUE),
            TileType::Empty => RGB::from_f32(0.0, 0.0, 0.0),
        }
    }
}
...
fn arb_rooms(
    ...
    let rooms = subsequence(generate_some_rooms(rng, mgc), mgc.num_rooms);
...
pub fn all_edges(paths: &Rect, mpg: &MapGenConfig) -> Vec<Rect> {
    vec![
        ...
        Rect::with_size(
            ...
            paths.y1,
            mpg.corridor_size,
            paths.height() + mpg.corridor_size,
        ),
        Rect::with_size(
            paths.x1,
            paths.y2,
...
pub fn fw_paths(path_vec: Vec<Rect>) -> Vec<Vec<Rect>> {
    vec![
        vec![path_vec[0], path_vec[1]],
        vec![path_vec[3], path_vec[2]],
    ]
}

pub fn bw_paths(path_vec: Vec<Rect>) -> Vec<Vec<Rect>> {
    vec![
        vec![path_vec[1], path_vec[2]],
        vec![path_vec[0], path_vec[3]],
    ]
}

pub fn connect_two_rooms(
    mpg: &MapGenConfig,
    ...
    match p1.x > p2.x {
        true => bw_paths(edges),
        false => fw_paths(edges),
    }
    .iter()
    .filter(|rects| 
        rects
        .iter()
        .all(|r| room_fits_in_map(r, mpg)))
    .choose(rng.get_rng())
    .unwrap_or(&vec![])
    .to_vec()
}

pub fn connect_some_rooms(
    mgc: &MapGenConfig,
    rng: &mut RandomNumberGenerator,
    origin: &Rect,
    rooms: &Vec<Rect>,
...
fn add_rect_to_map(rect: &Rect, map: &mut Map, floor_type: TileType, wall_type: TileType) {
    rect.point_set().iter().for_each(|p| {
        ...
        match (p.x, p.y) {
            (x, y) if x == rect.x1 || x == rect.x2 - 1 || y == rect.y1 || y == rect.y2 - 1 => {
                map.tiles[xy_idx(p.x, p.y)] = wall_type;
            }
            _ => {
                map.tiles[xy_idx(p.x, p.y)] = floor_type;
            }
        }
    });
}
...
pub fn add_doors(map: &mut Map) {
    ...
    for y in 1..map.height - 1 {
        ...
        for x in 1..map.width - 1 {
            ...
            match (
                map.tiles[xy_idx(x, y)],
                map.tiles[xy_idx(x - 1, y)],
                map.tiles[xy_idx(x + 1, y)],
                map.tiles[xy_idx(x, y - 1)],
                map.tiles[xy_idx(x, y + 1)],
            ) {
                (
                    TileType::Wall,
...
impl Map {
    ...
    pub fn reveal_map(&mut self) {
        for tile in self.revealed_tiles.iter_mut() {
            *tile = true;
        }
    }
